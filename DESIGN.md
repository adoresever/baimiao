...
白描是独立的通用中文改写与文采润色 Skill，清理 AI 痕迹只是其中一种用途。界面像一张正在被红笔处理的中文稿：米纸提供安静底面，墨蓝承载原意与结论，朱砂指出删改、失败和转向，青绿确认通过状态与方法检查。稿纸、删改线、重排纸条和细批注线都对应真实的改写动作，不作为泛复古装饰。
...
- **删改朱砂：**用于删除线、失败、流程箭头、方法选中态和收束整版；只在内容发生变化时出现。
...
**The 红笔有因 Rule.** 朱砂只表示删除、失败、选择或方向改变；普通强调不使用朱砂。
...
每轮测试是一条三列数据行：轮次、胶囊轨道、失败数。朱砂条表示仍有失败，最后一轮切换为青绿；条形从 5% 基点以 850ms 强减速曲线生长。减少动态偏好下将动画压缩至 0.01ms、单次执行。
...
- **Do** 用墨蓝保留原意，用朱砂标记删改与失败，用青绿确认检查与通过。
